...
export const OrganizationChart: React.FC = () => {
  ...
  const loadOrganizationalData = async () => {
    try {
      ...
      if (viewMode === 'teams' && canViewAllHierarchy) {
        ...
        const rootTeams: any[] = [];
        teams.forEach((t: any) => {
          if (t.parent_team_id && teamMap.has(t.parent_team_id)) {
            teamMap.get(t.parent_team_id).sub_teams.push(t);
          } else {
            rootTeams.push(t);
          }
        });

        data = transformTeamsToChartNodes(rootTeams);
  ...
  const transformTeamsToChartNodes = (teams: any[]): TeamChartNode[] => {
    return teams.map((team, index) => ({
      id: team.id,
      name: team.name,
      title: `Team - ${team.department_name || 'No Department'}`,
      department: team.department_name,
      email: team.lead_email,
      employee_id: team.lead_employee_id || '',
