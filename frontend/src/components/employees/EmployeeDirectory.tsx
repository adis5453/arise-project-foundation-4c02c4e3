...
const EmployeeDirectory: React.FC = () => {
  ...
  // Fetch positions for the MAIN TABLE filter
  const { data: positions = [] } = useQuery({
    queryKey: ['positions', departmentFilter],
    queryFn: () => DatabaseService.getPositions(departmentFilter),
  });

  // Fetch positions for the ADD/EDIT FORM (dependent on selected department)
  const { data: formPositions = [] } = useQuery({
    queryKey: ['positions', formData.department_id],
    queryFn: () => DatabaseService.getPositions(formData.department_id),
  ...
  // State for generated credentials modal
  ...
  const createEmployeeMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.generated_credentials) {
        setCreatedCredentials(data.generated_credentials);
      }
    },
    onError: (error: any) => {
      console.error("Create Error:", error);
      const msg = error.response?.data?.error || error.message || 'Failed to create employee. Please try again.';
      toast.error(msg);
    },
  });
  ...
  // Delete employee mutation (uses actual DELETE endpoint)
  const deleteEmployeeMutation = useMutation({
    mutationFn: async (employeeId: string) => {
      // Use the actual DELETE API endpoint
    ...
    onError: (error: any) => {
      ...
      toast.error(error?.message || 'Failed to remove employee. Please try again.');
    },
  });
  ...
  const handleConfirmDelete = useCallback(() => {
    ...
    // Use id (UUID) first, fall back to employee_id
    deleteEmployeeMutation.mutate(selectedEmployee.id || selectedEmployee.employee_id);
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter(employee => {
      ...
      const matchesStatus = !statusFilter || (employee as any).employment_status === statusFilter;
  ...
  const getPositionTitle = useCallback((positionId: string) => {
    const pos = positions.find((p: Position) => p.id === positionId) || formPositions.find((p: Position) => p.id === positionId);
    return pos?.name || 'No Position';
  ...
  return (
    <Box sx={{ p: responsive.getPadding(2, 3) }}>
          ...
          <TableBody>
            {filteredEmployees.map((employee) => (
              <TableRow key={employee.employee_id} hover sx={{ cursor: 'pointer' }} onClick={() => handleViewEmployee(employee)}>
                ...
                <TableCell>
                  <Chip
                    label={EMPLOYMENT_STATUSES.find(s => s.value === (employee as any).employment_status)?.label || (employee as any).employment_status}
                    color={EMPLOYMENT_STATUSES.find(s => s.value === (employee as any).employment_status)?.color as any || 'default'}
                    size="small"
                  />
                </TableCell>
                {!responsive.isMobile && (
      ...
      {/* Add Employee Dialog - Enhanced with Tabs */}
      <EnhancedEmployeeForm
        open={isAddDialogOpen}
        onClose={() => setIsAddDialogOpen(false)}
        onSubmit={(data) => {
          handleSubmitCreate({ preventDefault: () => { } } as React.FormEvent);
        }}
        formData={formData}
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <FormControl fullWidth>
                  ...
                  >
                    ...
                    {formPositions.map((pos: Position) => (
      ...
      >
        <DialogTitle>
          <Stack direction="row" alignItems="center" spacing={1}>
            ...
            <Typography variant="h6">Employee Credentials Created</Typography>
          </Stack>
        </DialogTitle>
        <DialogContent>
          ...
          <Typography variant="body2" color="text.secondary" paragraph>
            Share these credentials with the employee immediately. They will be required to change their password upon first login.
          </Typography>
          ...
          <Paper variant="outlined" sx={{ p: 2, bgcolor: 'grey.50' }}>
            <Stack spacing={2}>
              <Box>
                ...
                <Stack direction="row" alignItems="center" justifyContent="space-between">
                  ...
                  <IconButton
                    size="small"
                    onClick={() => {
                      navigator.clipboard.writeText(createdCredentials?.employee_id || '');
                      toast.success('Copied Employee ID');
                    }}
                  >
              ...
              <Box>
                <Typography variant="caption" color="text.secondary">Default Password</Typography>
                <Stack direction="row" alignItems="center" justifyContent="space-between">
                  <Typography variant="h6" sx={{ fontFamily: 'monospace' }}>
          ...
          <Alert severity="warning" icon={<WarningIcon />} sx={{ mt: 2 }}>
            ...
            This information will not be shown again. Please copy it now.
          </Alert>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setCreatedCredentials(null)} variant="contained">
            Done
          </Button>
        </DialogActions>
      </Dialog>
